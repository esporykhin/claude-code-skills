#!/bin/bash
# product-prices.sh — Get product prices from Ozon Seller API
# Usage: ./product-prices.sh [visibility] [limit] [last_id] [offer_ids_csv] [product_ids_csv]
...
if [ "${1:-}" = "--help" ]; then
  ...
  echo ""
  echo "Note: according to docs, max 1000 products per request."
  exit 0
fi
