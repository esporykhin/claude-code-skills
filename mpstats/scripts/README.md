...
| `wb/wb-category.sh` | Category product list (`POST wb/get/category`) | Product-level analysis in a WB category |
...
| `wb/wb-sku.sh` | SKU-level reports via `wb/get/item/{sku}/{report}` | Item sales/stocks/positions/reviews/similar |
| `wb/wb-search.sh` | Subjects/niches list (`wb/get/subjects/select`) | Niche discovery and subject lookup |
| `wb/wb-subject.sh` | Subject-level methods (`wb/get/subject/*`, except `geography`) | Subject analytics by subject id |
| `wb/wb-similar.sh` | Similar families (`identical`, `similar`, `in_similar`) | Similar product analysis for SKU/url |
| `wb/wb-analytics.sh` | Forecast and season effects (`analytics/v1/wb/*`) | AI forecasts and seasonality |
| `wb/wb-promotion-analysis.sh` | Subject promotion analysis | Promo effect analysis for WB subject |
| `wb/wb-warehouses.sh` | Warehouse breakdown (`wb/get/brand|seller/in_warehouses`) | Stock distribution by warehouse |
