...
if [ "$KIND" = "brand" ]; then
  ...
  URL="https://mpstats.io/api/wb/get/brand/in_warehouses?d1=${D1}&d2=${D2}&path=${ENCODED}"
elif [ "$KIND" = "seller" ]; then
  URL="https://mpstats.io/api/wb/get/seller/in_warehouses?d1=${D1}&d2=${D2}&path=${VALUE}"
else
