...
if [ -z "$1" ] || [ -z "$2" ] || [ "$1" = "--help" ]; then
  ...
  echo "  family     — identical | similar | in_similar"
  echo "  path_value — SKU or product URL"
  echo "  report     — products (default), categories, brands, sellers, price_segmentation"
...
if [ "$FAMILY" != "identical" ] && [ "$FAMILY" != "similar" ] && [ "$FAMILY" != "in_similar" ]; then
  echo "family must be one of: identical | similar | in_similar" >&2
  exit 1
fi
...
BASE_URL="https://mpstats.io/api/wb/get/${FAMILY}"
