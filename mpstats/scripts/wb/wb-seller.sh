...
if [ -z "$1" ] || [ "$1" = "--help" ]; then
  ...
  echo "  supplier_id — WB seller/supplier ID (numeric, sent as API param: path)"
...
if [ "$REPORT" = "products" ]; then
  curl -s --location --request POST \
    "https://mpstats.io/api/wb/get/seller?d1=${D1}&d2=${D2}&path=${SUPPLIER_ID}&fbs=${FBS}" \
    --header "X-Mpstats-TOKEN: $TOKEN" \
    --header 'Content-Type: application/json' \
    --data "{\"startRow\":0,\"endRow\":${LIMIT},\"filterModel\":{},\"sortModel\":[{\"colId\":\"revenue\",\"sort\":\"desc\"}]}"
else
  curl -s --location --request GET \
    "https://mpstats.io/api/wb/get/seller/${REPORT}?d1=${D1}&d2=${D2}&path=${SUPPLIER_ID}&fbs=${FBS}" \
    --header "X-Mpstats-TOKEN: $TOKEN" \
    --header 'Content-Type: application/json'
fi
