...
QUERY="d1=${D1}&d2=${D2}&path=${ENCODED_BRAND}&brand=${ENCODED_BRAND}&fbs=${FBS}"
if [ -n "$NEWSMODE" ]; then
...
if [ "$REPORT" = "products" ]; then
  RESPONSE=$(curl -s --location --request POST \
    "https://mpstats.io/api/oz/get/brand?${QUERY}" \
    --header "X-Mpstats-TOKEN: $TOKEN" \
    --header 'Content-Type: application/json' \
    --data "{\"startRow\":0,\"endRow\":${LIMIT},\"filterModel\":{},\"sortModel\":[{\"colId\":\"revenue\",\"sort\":\"desc\"}]}")

  # Some docs mention oz/get/brands, so keep a fallback for compatibility.
  if printf '%s' "$RESPONSE" | grep -Eq '"code":[ ]*404|Not Found'; then
    RESPONSE=$(curl -s --location --request POST \
      "https://mpstats.io/api/oz/get/brands?${QUERY}" \
      --header "X-Mpstats-TOKEN: $TOKEN" \
      --header 'Content-Type: application/json' \
      --data "{\"startRow\":0,\"endRow\":${LIMIT},\"filterModel\":{},\"sortModel\":[{\"colId\":\"revenue\",\"sort\":\"desc\"}]}")
  fi
  ...
  curl -s --location --request GET \
    "https://mpstats.io/api/oz/get/brand/${REPORT}?${QUERY}" \
    --header "X-Mpstats-TOKEN: $TOKEN" \
    --header 'Content-Type: application/json'
fi
