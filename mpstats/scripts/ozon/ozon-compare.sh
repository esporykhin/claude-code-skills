#!/bin/bash
# ozon-compare.sh — Compare two periods for category/brand/seller
# Usage: ./ozon-compare.sh <scope> <value> <d11> <d12> <d21> <d22> [d1] [d2] [fbs] [limit]
...
if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ] || [ -z "$4" ] || [ -z "$5" ] || [ -z "$6" ] || [ "$1" = "--help" ]; then
  ...
  echo "  value — category path, brand name, seller_id or seller name"
  exit 0
fi
...
BASE_URL="https://mpstats.io/api/oz/get"
...
case "$SCOPE" in
  ...
  brand)
    ...
    URL="$BASE_URL/brand/compare?${QUERY}&path=${ENCODED}&brand=${ENCODED}"
    ;;
  seller)
    if printf '%s' "$VALUE" | grep -Eq '^[0-9]+$'; then
      URL="$BASE_URL/seller/compare?${QUERY}&seller_id=${VALUE}"
    else
      ENCODED=$(python3 -c "import urllib.parse,sys; print(urllib.parse.quote(sys.argv[1]))" "$VALUE" 2>/dev/null || node -e "process.stdout.write(encodeURIComponent(process.argv[1]))" "$VALUE" 2>/dev/null || printf '%s' "$VALUE" | sed 's/ /%20/g')
